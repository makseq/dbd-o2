...
import { t as tx, LANG, applyStatic } from './i18n.js';
...
// ── локализация чисел (I18N.md §4) ────────────────────────────────────────
// ru: запятая-десятичная + неразрывный пробел в тысячах («5 480», «0,658»)
// en: точка-десятичная + запятая в тысячах («5,480», «0.658»)
// Через loc() проходит ТОЛЬКО текст, попадающий на экран. Значения <input>,
// CSV/JSON-экспорт и JSON.stringify(params) остаются машиночитаемыми.
const DECSEP = LANG === 'ru' ? ',' : '.';
const GRPSEP = LANG === 'ru' ? '\u00A0' : ',';
/** ASCII-число («-1234.56», «1.2e-7», 4096) → строка на активном языке. */
function loc(v) {
  const s = String(v);
  const m = /^([+\-\u2212]?)(\d+)(?:\.(\d+))?([eE][+\-]?\d+)?$/.exec(s);
  if (!m) return s;
  const int = m[2].length > 3 ? m[2].replace(/\B(?=(\d{3})+$)/g, GRPSEP) : m[2];
  return m[1] + int + (m[3] ? DECSEP + m[3] : '') + (m[4] || '');
}
...
function sci(v, digits = 2) {
  ...
  if (e >= -2 && e <= 3) return s + loc(Number(a.toPrecision(3)).toString());
  return `${s}${loc(m.toFixed(digits))}·10${supStr(e)}`;
}
...
const sig3 = (v) => (isFinite(v) ? (Math.abs(v) >= 1000 || (Math.abs(v) < 0.01 && v !== 0) ? sci(v) : loc(Number(v.toPrecision(3)).toString())) : '–');
const NBSP = ' ';
...
const SPARK_N = 64;
// [ключ метрики, ключ подписи в словаре, ключ единицы в словаре]
const METRICS = [
  ['power', 'metric.power', 'unit.W'], ['spec', 'metric.spec', 'unit.JL'], ['o3', 'metric.o3', 'unit.ppm'],
  ['en', 'metric.en', 'unit.Td'], ['ipk', 'metric.ipk', 'unit.mA'], ['bd', 'metric.bd', ''],
  ['cdiel', 'metric.cdiel', 'unit.pF'], ['ccell', 'metric.ccell', 'unit.pF'], ['eper', 'metric.eper', 'unit.mJ'],
];
...
// ═══════════════════════════════════════════════════════════ controls ═══════
// lk / uk — ключи подписи и единицы измерения в словаре i18n.js
const CONTROLS = [
  { k: 'U0kV', lk: 'ctl.U0kV', min: 1, max: 20, step: 0.1, uk: 'unit.kV', log: false },
  { k: 'freqKHz', lk: 'ctl.freqKHz', min: 0.5, max: 100, step: 0.01, uk: 'unit.kHz', log: true },
  { k: 'gapMM', lk: 'ctl.gapMM', min: 0.2, max: 5, step: 0.01, uk: 'unit.mm', log: true, geom: true },
  { k: 'dielMM1', lk: 'ctl.dielMM1', min: 0.1, max: 2, step: 0.05, uk: 'unit.mm', geom: true, twin: 'dielMM2' },
  { k: 'epsR', lk: 'ctl.epsR', min: 1, max: 30, step: 0.5, uk: '' },
  { k: 'gamma', lk: 'ctl.gamma', min: 0, max: 0.3, step: 0.005, uk: '' },
  { k: 'seedDensity', lk: 'ctl.seedDensity', min: 1e10, max: 1e16, step: 0.01, uk: 'unit.m3', log: true },
  { k: 'pressureTorr', lk: 'ctl.pressureTorr', min: 76, max: 1520, step: 1, uk: 'unit.Torr' },
  { k: 'tempK', lk: 'ctl.tempK', min: 250, max: 600, step: 10, uk: 'unit.K' },
  { k: 'areaCM2', lk: 'ctl.areaCM2', min: 0.1, max: 100, step: 0.01, uk: 'unit.cm2', log: true },
  { k: 'ballastOhm', lk: 'ctl.ballastOhm', min: 0, max: 1e7, step: 1, uk: 'unit.ohm' },
  { k: 'nCells', lk: 'ctl.nCells', min: 100, max: 2000, step: 1, uk: '', log: true, hard: true },
];
const ctlEls = {};
...
function buildControls() {
  ...
  for (const c of CONTROLS) {
    ...
    const cLabel = tx(c.lk), cUnit = c.uk ? tx(c.uk) : '';
    lab.textContent = cLabel;
    ...
    rng.setAttribute('aria-label', `${cLabel}${cUnit ? ', ' + cUnit : ''}`);
    num.setAttribute('aria-label', tx('ctl.valueAria', { label: cLabel, unit: cUnit ? ', ' + cUnit : '' }));
    num.title = tx('ctl.rangeTitle', { label: cLabel, min: loc(c.min), max: loc(c.max), unit: cUnit ? ' ' + cUnit : '' });
    ...
    u.className = 'unit'; u.textContent = cUnit;
  ...
  sec.textContent = tx('ctl.secAccuracy');
  host.append(sec);
  const row = document.createElement('div');
  row.className = 'ctl';
  const lab = document.createElement('label'); lab.textContent = tx('ctl.mode');
  const sel = document.createElement('select');
  for (const m of ['demo', 'default', 'accurate']) {
    const o = document.createElement('option'); o.value = m; o.textContent = tx('ctl.mode.' + m); sel.append(o);
  }
  ...
  sel.setAttribute('aria-label', tx('ctl.modeAria'));
...
function onParamsChanged(hard) {
  ...
  $('hdrSummary').textContent = tx('hdr.summary',
    { u: loc(p.U0kV.toFixed(1)), f: loc(p.freqKHz.toFixed(1)), gap: loc(p.gapMM.toFixed(2)), eps: loc(p.epsR) });
  updateGridInfo();
  ...
  if (hard) { resetUiState(); announce(tx('msg.geometryChanged')); }
}
...
function updateGridInfo() {
  ...
  el.textContent = tx('grid.info', {
    mark: bad ? '✕ ' : warn ? '⚠ ' : '✓ ', nb: NBSP,
    dx: sig3(dx * 1e6), ld: sig3(lD * 1e6), now: sig3(ratio), worst: sig3(gridWorst),
  });
}
...
function buildMetrics() {
  const host = $('metrics');
  host.textContent = '';
  for (const [k, labelKey, unitKey] of METRICS) {
    const d = document.createElement('div');
    d.className = 'tile';
    const kk = document.createElement('div'); kk.className = 'k'; kk.textContent = tx(labelKey);
    const v = document.createElement('div'); v.className = 'v mono';
    v.innerHTML = `<span class="n">–</span>${unitKey ? `<span class="u">${tx(unitKey)}</span>` : ''}`;
...
function buildLegend() {
  ...
  hint.textContent = tx('legend.hint');
  host.append(hint);
  syncLegend();
}
function syncLegend() {
...
// ═══════════════════════════════════════════════════════════ field access ═══
const FIELDS = {
  ne: { name: tx('field.ne'), unit: tx('field.ne.unit'), log: true, signed: false, get: (st) => st.n.e },
  rho: { name: tx('field.rho'), unit: tx('field.rho.unit'), log: false, signed: true, get: (st) => st.rho },
  ...
  sion: {
    name: tx('field.sion'), unit: tx('field.sion.unit'), log: true, signed: false,
    get: (st) => st.ionizRate, paint: () => buf.glowAbs || null,
  },
  Eabs: { name: tx('field.Eabs'), unit: tx('field.Eabs.unit'), log: false, signed: false, get: (st) => st.E, abs: true },
  EN: { name: tx('field.EN'), unit: tx('field.EN.unit'), log: false, signed: false, get: (st) => st.EN },
};
...
function drawGap() {
  ...
  if (V.gapMode === 'snapshot') {
    ...
    for (let k = 0; k <= 4; k++) {
      ...
      text(ctx, k === 4 ? tx('gap.now') : `−${fmtTime(lens * (1 - k / 4), 1)}`,
    ...
    text(ctx, tx('gap.streakInfo', { row: fmtTime(dtRow, 1), v: frontSpeedText() }),
  ...
  const drawSigma = (xpos, sigma, side) => {
    ...
    text(ctx, tx('gap.sigma', { v: sig3(sigma * 1e5) }), xpos, y0 + bandH + 11, { color: COL.text2, size: 9.5, align: side === 'L' ? 'left' : 'right' });
  };
  ...
  for (let i = 0; i <= nTicks; i++) {
    ...
    text(ctx, loc(((i / nTicks) * p.gapMM).toFixed(2)), xx, ay + 6, { size: 9, align: 'center', base: 'top' });
  }
  text(ctx, tx('gap.axisX'), gx0 + wg + 4, ay + 6, { size: 9, base: 'top' });
  text(ctx, tx('gap.metal'), x0 + metalW / 2, y0 - 3, { size: 8.5, align: 'center', base: 'bottom' });
  text(ctx, tx('gap.dielectric'), x0 + metalW + wd1 / 2, y0 - 3, { size: 8.5, align: 'center', base: 'bottom' });
  text(ctx, tx('gap.gasGap'), gx0 + wg / 2, y0 - 3, { size: 8.5, align: 'center', base: 'bottom', color: COL.text2 });
  ...
  text(ctx, tx('gap.colorbar', { name: f.name, unit: f.unit }), cbx - 2, y0 - 4, { size: 9, align: 'right', base: 'bottom', color: COL.text2 });
  ...
  if (gapHover >= 0) {
    ...
    $('gapReadout').textContent = tx('gap.readout',
      { name: f.name, x: loc(xmm.toFixed(3)), v: sci(raw), unit: f.unit });
...
function frontSpeedText() {
  ...
  return frontHist.v > 0 ? tx('gap.frontSpeed', { v: sci(frontHist.v) }) : tx('gap.frontNA');
}
...
function drawWaves() {
  ...
  for (const v of [-U0 / 1.25, 0, U0 / 1.25]) {
    text(ctx, loc((v / 1e3).toFixed(0)), padL - 4, yOfU(v), { size: 9, align: 'right', base: 'middle' });
  }
  ...
  // ── I track ──
  frame(yI, hI, tx(V.iLog ? 'wave.trackILog' : 'wave.trackI'));
  const st = solver.state;
  ...
  if (V.iLog) {
    ...
    for (const v of [-m / 1.2, 0, m / 1.2]) text(ctx, loc((v * 1e3).toFixed(1)), padL - 4, yOfI(v), { size: 9, align: 'right', base: 'middle' });
  ...
  ctx.setLineDash([]);
  ctx.restore();
  text(ctx, tx(V.showItot ? 'wave.iBoth' : 'wave.iOne'), padL + pw - 4, yI + 3, { size: 9, align: 'right', base: 'top', color: '#ffd166' });
  ...
  if (res.samples === 0) {
    const hs = historySpan();
    text(ctx, hs.span > 0
      ? tx('wave.noSamplesWindow', { span: fmtTime(hs.span) })
      : tx('wave.noSamplesYet'),
      padL + pw / 2, yU + hU / 2, { size: 11.5, align: 'center', base: 'middle', color: COL.muted, mono: false });
  }
  $('waveCursor').textContent = tx('wave.cursor', { t: fmtTime(V.cursorT, 4) });
  const dec = res.samples > res.bins ? Math.round(res.samples / res.bins) : 0;
  const badge = $('decBadge');
  badge.hidden = dec < 2;
  if (dec >= 2) badge.textContent = tx('wave.decimated', { n: dec });
}
function fmtTime(tv, digits = 3) {
  const a = Math.abs(tv);
  if (a >= 1e-3) return `${loc((tv * 1e3).toFixed(digits))} ${tx('unit.ms')}`;
  if (a >= 1e-6) return `${loc((tv * 1e6).toFixed(digits))} ${tx('unit.us')}`;
  return `${loc((tv * 1e9).toFixed(1))} ${tx('unit.ns')}`;
}
...
function drawQV() {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    text(ctx, loc((u / 1e3).toFixed(0)), X(u), py0 + ph + 4, { size: 9, align: 'center', base: 'top' });
    const q = -qmax + (i / 4) * 2 * qmax;
    text(ctx, loc((q * 1e9).toFixed(1)), px0 - 4, Y(q), { size: 9, align: 'right', base: 'middle' });
  }
  text(ctx, tx('qv.axisU'), px0 + pw, py0 + ph + 15, { size: 9, align: 'right', base: 'top', color: COL.text2 });
  text(ctx, tx('qv.axisQ'), px0 - 4, py0 - 2, { size: 9, align: 'right', base: 'bottom', color: COL.text2 });
  ...
  $('qvFoot').innerHTML = tx('qv.foot', {
    nb: NBSP,
    cd: sig3(Cd * 1e12), cdg: sig3((ps.CdielGeom || 0) * 1e12), cc: sig3(Cc * 1e12),
    e: sig3(E * 1e3), pw: sig3(ps.powerW || 0), ub: sig3(ps.UburnkV || 0),
    ghosts: V.qvWindow === 8 ? tx('qv.ghosts', { n: ghostsShown }) : '',
  });
  ...
  if (bad) {
    warn.textContent = (ps.qvFallback !== false || !(ps.qvArcsOn > 0))
      ? tx('qv.warnSides', { on: ps.qvArcsOn || 0, off: ps.qvArcsOff || 0 })
      : tx('qv.warnShape', {
        r2on: sig3(ps.qvR2on || 0), r2off: sig3(ps.qvR2off || 0),
        on: ps.qvArcsOn, off: ps.qvArcsOff,
      });
    warn.title = tx('qv.warnTitle');
  }
}
...
function drawProfiles() {
  ...
  if (isX) { count = nx; X = (k) => padL + gpos(st, gidx(k)) * pw; }
    ...
    count = tsT.n;
    if (count < 2) {
      text(ctx, tx('prof.collecting'), padL + pw / 2, padT + ph / 2,
  ...
  for (let i = 0; i <= 4; i++) {
    const x = padL + (i / 4) * pw;
    let lab;
    if (isX) lab = loc(((i / 4) * gapMM).toFixed(2));
    else if (tsT.n < 2) lab = '–';
    else {
  ...
  text(ctx, tx(isX ? 'prof.axisX' : 'prof.axisT'), padL + pw + 4, padT + ph + 4, { size: 9, base: 'top', color: COL.text2 });
  ...
  if (profHover >= 0 && isX) {
    ...
    const tip = $('profTip');
    tip.hidden = false;
    tip.textContent = tx('prof.tip', {
      x: loc(((st.x[i] - GAS.x0) * 1e3).toFixed(3)),
      rows: rows.map((r) => `${r.s.label.padEnd(7)} ${sci(r.v)}`).join('\n'),
      q: sci(qsum),
    });
    const bx = Math.min(o.w - 150, x + 10);
    tip.style.left = bx + 'px'; tip.style.top = padT + 4 + 'px';
    $('profCursor').textContent = tx('prof.cursor', { x: loc(((st.x[i] - GAS.x0) * 1e3).toFixed(3)) });
  } else { $('profTip').hidden = true; $('profCursor').textContent = tx('prof.cursorInit'); }
}
...
function drawMacro() {
  ...
  if (hs.span > 0 && hs.t0 > t0) {
    ...
    if (X(hs.t0) > 60) text(ctx, tx('macro.noHistory'), 4, 9, { size: 8.5, base: 'top' });
  }
  ...
  text(ctx, tx('macro.total', { t: fmtTime(t1), n: Math.floor(nP) }), W - 3, H - 3, { size: 9, align: 'right' });
  macroGeom = { t0, t1, W };
}
let macroGeom = null;
  ...
  for (let s = 0; s < h.len; s++) {
    ...
    if (a > thr && evPrevI <= thr) {
      ...
      if (t - last > 5e-9) {
        ...
        announce(tx('msg.breakdownAt', { t: fmtTime(t) }));
      }
    }
    evPrevI = a;
    evScanT = t;
...
function updateMetrics(force) {
  ...
  const o3 = withUnit(st.o3ppm || 0, tx('unit.ppm'), [[1e4, tx('unit.pct')]]);
  set('o3', o3.unit === tx('unit.pct') ? sig3(o3.v / 100) : sig3(o3.v), o3.unit);
  set('en', sig3(st.maxEN || 0));
  const ipk = withUnit((st.peakCurrent || 0) * 1e3, tx('unit.mA'), [[1e3, tx('unit.A')]]);
  ...
  if (pIdx !== lastPeriodIdx) {
    ...
    $('panelMetrics').setAttribute('aria-label', tx('metrics.periodAria', {
      i: loc(pIdx), bd: loc(st.breakdownsPerPeriod || 0),
      ipk: sig3((st.peakCurrent || 0) * 1e3), pw: sig3(ps.powerW || 0),
    }));
    if (V.steady) checkSteady(ps);
  }
  updateGridInfo();   // 4 Гц, по ТЕКУЩЕМУ n_e (а не раз в период, в тихой фазе)
  $('periodText').textContent = tx('metrics.period', { i: loc(pIdx), t: fmtTime(st.t) });
}
...
function checkSteady(ps) {
  ...
  if (steadyHist.length === 2) {
    ...
    if (steadyCount >= 5) {
      ...
      announce(tx('msg.steady', { d: loc((rel * 100).toFixed(2)) }));
    }
  }
}
...
function updateStatus(steps, simDt, wallMs) {
  ...
  el.style.color = cls;
  el.textContent = tx('solver.status', {
    dt: sci(st.dt), steps: loc(steps), wall: loc(wallMs.toFixed(1)), sim: fmtTime(simDt),
    rej: sig3(rejFrac * 100),
    resid: res !== null ? tx('solver.resid', { v: sci(res) }) : '',
    clip: tx('solver.clip', { v: sci(st.clipCharge || 0) }),
  });
}
...
function frame() {
  ...
  if (now - fpsT > 500) {
    ...
    $('fpsText').textContent = tx('hdr.fps', { n: loc(fps.toFixed(0)) });
  }
}
...
function setRunning(on) {
  V.running = on;
  $('btnPlay').textContent = tx(on ? 'tr.pause' : 'tr.play');
  $('runBadge').classList.toggle('running', on);
  $('runText').textContent = tx(on ? 'run.running' : 'run.paused');
}
function updateSpeedLabel() {
  const s = Math.pow(10, V.speedLog);
  $('speedText').textContent = tx('tr.speedBadge', { v: sci(s) });
  $('speedLabel').textContent = tx('tr.speedLabel', { v: sci(s), t: fmtTime(s) });
}
function updateLensLabel() {
  const w = Math.pow(10, V.lensLog);
  const T = 1 / (solver.params.freqKHz * 1e3);
  $('lensLabel').textContent = tx('tr.lensLabel', { w: fmtTime(w), n: sig3(w / T) });
}
...
function snapEvent(dir) {
  ...
  else announce(tx('msg.noMoreEvents'));
}
...
function showTable(which) {
  ...
  if (which === 'prof' || which === 'gap') {
    ...
    let rows = `<table><tr><th>${tx('table.th.xmm')}</th>${keys.map((k) => `<th>${k}</th>`).join('')}`
      + `<th>${tx('table.th.E')}</th><th>${tx('table.th.EN')}</th><th>${tx('table.th.rho')}</th></tr>`;
    const stride = Math.max(1, Math.floor(GAS.n / 60));
    for (let i = GAS.i0; i <= GAS.i1; i += stride) {
      rows += `<tr><td>${loc(((st.x[i] - GAS.x0) * 1e3).toFixed(4))}</td>${keys.map((k) => `<td>${sci(st.n[k][i])}</td>`).join('')}` +
        `<td>${sci(st.E[i])}</td><td>${sig3(st.EN[i])}</td><td>${sci(st.rho[i])}</td></tr>`;
    }
    openModal(tx('table.profTitle', { t: fmtTime(st.t) }), rows + '</table>');
  } else if (which === 'wave') {
    const h = solver.history;
    const start = (h.head - h.len + h.capacity) % h.capacity;
    const stride = Math.max(1, Math.floor(h.len / 200));
    let rows = `<table><tr><th>${tx('table.th.t')}</th><th>${tx('table.th.Uapp')}</th>`
      + `<th>${tx('table.th.Ugap')}</th><th>${tx('table.th.I')}</th><th>${tx('table.th.Q')}</th></tr>`;
    for (let s = 0; s < h.len; s += stride) {
      const i = (start + s) % h.capacity;
      rows += `<tr><td>${loc(h.t[i].toExponential(6))}</td><td>${loc(h.Uapp[i].toFixed(1))}</td><td>${loc(h.Ugap[i].toFixed(1))}</td><td>${loc(h.current[i].toExponential(3))}</td><td>${loc(h.charge[i].toExponential(3))}</td></tr>`;
    }
    openModal(tx('table.waveTitle'), rows + '</table>');
  } else {
    const ps = solver.periodStats || {};
    openModal(tx('table.manleyTitle'), tx('table.manleyBody', {
      cd: sig3((ps.Cdiel || 0) * 1e12), cc: sig3((ps.Ccell || 0) * 1e12),
      ub: sig3(ps.UburnkV || 0), e: sig3((ps.energyPerPeriodJ || 0) * 1e3),
      pw: sig3(ps.powerW || 0), params: JSON.stringify(solver.params),
    }));
  }
}
$('btnHelp').addEventListener('click', () => openModal(tx('help.title'), tx('help.body')));
...
const paramHeader = () => {
  const hs = historySpan();
  return `# dbd-o2 v0.1 ${USE_MOCK ? tx('export.mock') : ''}\n`
    + `# ${new Date().toISOString()}\n`
    + `${tx('export.params')} ${JSON.stringify(solver.params)}\n`
    + tx('export.header', {
      span: (hs.span * 1e6).toFixed(2),
      periods: (hs.span * solver.params.freqKHz * 1e3).toFixed(2),
      total: (solver.state.t * 1e6).toFixed(2),
    });
};
...
$('btnPNG').addEventListener('click', () => {
  const list = [[tx('panel.gap'), cvGap], [tx('panel.wave'), cvWave], [tx('panel.qv'), cvQV], [tx('panel.prof'), cvProf]];
  ...
  const p = solver.params;
  text(g, tx('png.footer', {
    mock: USE_MOCK ? ' [MOCK]' : '', u: p.U0kV, f: p.freqKHz, gap: p.gapMM,
    eps: p.epsR, gamma: p.gamma, t: fmtTime(solver.state.t), iso: new Date().toISOString(),
...
// ═══════════════════════════════════════════════════════════ boot ══════════
applyStatic();          // статическая разметка index.html + <title> + переключатель
buildControls();
buildMetrics();
buildLegend();
