// plots.js — графики плеера 2D-ДБР (canvas 2D, тёмная тема, ретина).
//
// Классы:
...
//
// Зависимости: только ./metrics.js. Модуль не знает про index.html/app.js.
...
class BasePlot {
  constructor(canvas, opts = {}) {
    if (!canvas) throw new Error(tr('plots.noCanvas'));
    this.canvas = canvas;
 ...
 * @param {object} series  колоночные ряды из loader.js (t, Uapp, Ugap, Icond, Idisp, Itot, ...)
...
export class WaveformPlot extends BasePlot {
  ...
  draw(g) {
    if (!this.nS) { drawEmpty(g, this, tr('plots.noSeries')); return; }
    ...
    // ── оси
    this._xLabels(g, xs, this.xt, tr('plots.axisT', { unit: tScale.unit }));
    this._yLabels(g, ys, this.yt, tr('plots.axisU'), 'left', THEME.textSecondary);
    const yIs = this.logCurrent ? this.yI : dispScale(this.yIs, this.yI, 1e3);
    this._yLabels(g, yIs, this.ytI, tr(this.logCurrent ? 'plots.axisIlog' : 'plots.axisI'), 'right', THEME.Icond);
    ...
    if (this.logCurrent) {
      ...
      g.fillText(tr('plots.logNote'), this.px0 + 3, this.py0 - 4);
    }
  }
...
export class LissajousPlot extends BasePlot {
  ...
  draw(g) {
    if (!this.nS) { drawEmpty(g, this, tr('plots.noSeries')); return; }
    if (!this.series.Q || !this.series.Uapp) { drawEmpty(g, this, tr('plots.noQU')); return; }
    ...
    this._xLabels(g, xs, this.xt, tr('plots.axisUapp'));
    this._yLabels(g, ys, this.yt, tr('plots.axisQ', { unit: qk.unit }), 'left', THEME.textSecondary);
  ...
  _report(g, fit, x, p) {
    ...
    if (fit && fit.ok) {
      ...
      if (an) this._line(g, tr('plots.vsAnalytic'), fmtSI(an.C_cell, 'F') + '  ' + fmtPct(dCell), devColor(dCell, 4, 25));
      this._line(g, 'C_diel fit', fmtSI(fit.C_diel, 'F'), devColor(dDiel, 4, 25));
      if (an) this._line(g, tr('plots.vsAnalytic'), fmtSI(an.C_diel, 'F') + '  ' + fmtPct(dDiel), devColor(dDiel, 4, 25));
      this._line(g, tr('plots.r2sides'), fmtFixed(fit.r2min, 3), fit.r2min < 0.98 ? THEME.warning : THEME.text);
    } else {
    ...
    if (fit) {
      ...
      this._line(g, tr('plots.closure'), fit.closure != null ? fmtPct(100 * fit.closure, 2) : '—',
...
export function fitLissajous(U, Q, i0, i1, T, analytic) {
  ...
  const n = i1 - i0 + 1;
  if (n < 16) { out.warn = tr('fit.shortPeriod'); return out; }
  ...
  if (cnt < 16) { out.warn = tr('fit.noSegments'); return out; }
  ...
  out.ok = true;
  const warns = [], notes = [];
  if (out.closure != null && Math.abs(out.closure) > 0.05) warns.push(tr('fit.loopOpen'));
  if (out.r2min < 0.98) warns.push(tr('fit.lowR2', { v: out.r2min.toFixed(3) }));
  // информационно, НЕ бракует наклон: медиана локального dQ/dU от дробления не страдает
  if (out.cellSpread > 0.05) {
    notes.push(tr('fit.fragmented', { v: (100 * out.cellSpread).toFixed(0) }));
  }
  if (!(out.clusterSep > 1.5)) warns.push(tr('fit.notSeparated', { v: out.clusterSep.toFixed(2) }));
  if (cellSides.length < 2 || dielSides.length < 2) warns.push(tr('fit.fewSides'));
  if (analytic) {
    const d = Math.max(Math.abs(out.C_cell / analytic.C_cell - 1), Math.abs(out.C_diel / analytic.C_diel - 1));
    if (d > 0.25) warns.push(tr('fit.deviation', { v: (100 * d).toFixed(0) }));
  }
  if (warns.length) out.warn = warns.join(' · ') + tr('fit.unreliable');
...
export class RadialProfilePlot extends BasePlot {
  ...
  draw(g) {
    if (!this.r || !this.sL) { drawEmpty(g, this, tr('plots.noFrame')); return; }
    ...
    this._xLabels(g, this.x, this.xt, tr('plots.axisR'));
    this._yLabels(g, this.y, this.yt, tr('plots.axisSigma'), 'left', THEME.textSecondary);

    // легенда + числа на оси
    g.font = THEME.fontMonoSmall;
    g.textAlign = 'left'; g.textBaseline = 'top';
    g.fillStyle = THEME.sigmaL;
    g.fillText(tr('plots.sigmaLo', { v: fmtFixed(this.sL[0] / K, 2) }), this.px0 + 4, this.py1 + 3);
    if (this.sR) {
      g.fillStyle = THEME.sigmaR;
      g.fillText(tr('plots.sigmaHi', { v: fmtFixed(this.sR[0] / K, 2) }), this.px0 + 4, this.py1 + 15);
    }
    if (this.scaleMode === 'asinh') {
      g.fillStyle = THEME.textMuted;
      g.textAlign = 'right';
      g.fillText(tr('plots.asinhNote'), this.px1 - 4, this.py1 + 3);
    }
  }
...
/**
 * Реестр сортов. Поле `field` — имя в frames.bin (recorder.js defaultFields).
 *
...
export class AxialProfilePlot extends BasePlot {
  ...
  draw(g) {
    if (!this.z) { drawEmpty(g, this, tr('plots.noFrame')); return; }
    ...
    this._xLabels(g, this.x, this.xt, tr('plots.axisZ'));
    this._yLabels(g, this.y, this.yt, tr('plots.axisN'), 'left', THEME.textSecondary);
    if (clipped) {
      g.fillStyle = THEME.textMuted;
      g.font = THEME.fontMonoSmall;
      g.textAlign = 'right'; g.textBaseline = 'bottom';
      g.fillText(tr('plots.clipped', { n: clipped, lo: fmtExp(lo) }), this.px1, this.py0 - 3);
    }
    this._drawLegend(g);
  }
  ...
  _drawLegend(g) {
    ...
    g.fillStyle = THEME.textMuted;
    g.fillText(tr('plots.axialHint'), this.px0, y + 13);
  }
...
export const CHANNELS = [
  { key: 'photoIon', label: tr('chan.photoIon'), color: '#ffd166', dash: null, width: 2,
    source: 'field', field: 'photoIonRate', unit: 's⁻¹' },
  { key: 'photoEmit', label: tr('chan.photoEmit'), color: '#4da3ff', dash: [5, 2], width: 2,
    source: 'series', series: ['photoEmitTotalL', 'photoEmitTotalR'], unit: 's⁻¹' },
  { key: 'photoDetach', label: tr('chan.photoDetach'), color: '#9085e9', dash: [8, 3, 2, 3], width: 2,
    source: 'field', field: 'photoDetachRate', unit: 's⁻¹' },
  { key: 'impactIon', label: tr('chan.impactIon'), color: '#e66767', dash: null, width: 1.6,
    source: 'field', field: 'ionizRate', unit: 's⁻¹', reference: true },
  { key: 'collDetach', label: tr('chan.collDetach'), color: '#199e70', dash: [1, 3], width: 1.6,
    source: 'none', unit: 's⁻¹', reference: true,
    note: tr('chan.collDetach.note') },
];
...
export class PhotoSeriesBuilder {
  /**
   * @param {import('./loader.js').Playback} pb
...
export class PhotoPanel extends BasePlot {
  ...
  draw(g) {
    const d = this.data;
    if (!d) { drawEmpty(g, this, tr('plots.noPhotoSeries')); return; }
    const n = d.t.length;
    ...
    this._xLabels(g, xs, this.xt, tr('plots.axisT', { unit: ts.unit }));
    this._yLabels(g, this.y, this.yt, tr('plots.photoAxis'), 'left', THEME.textSecondary);
    ...
    if (Number.isFinite(this.cursorT) && d.chan.photoIon && d.chan.impactIon) {
      ...
      g.fillText(tr('plots.photoIonRate', { v: fmtExp(pi, 's⁻¹', 2) }), this.px1 - 3, this.py1 + 15);
    }
    if (zeros) {
      g.font = THEME.fontMonoSmall;
      g.textAlign = 'right'; g.textBaseline = 'bottom';
      g.fillStyle = THEME.textMuted;
      g.fillText(tr('plots.zeros', { n: zeros }), this.px1 - 3, this.py0 - 3);
    }
    this._drawLegend(g);
  }
  ...
  _drawLegend(g) {
    ...
    g.fillStyle = THEME.textMuted;
    g.fillText(tr('plots.photoHint'), this.px0, y + 13);
  }
