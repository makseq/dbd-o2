// app.mjs — управляющий слой плеера: загрузка прогонов, состояние, транспорт,
// двухмасштабный таймлайн, режим сравнения «фото вкл / фото выкл».

import Playback from './loader.mjs';
import Scene from './scene.mjs';
import { COLORMAP_NAMES, COLORMAP_LABELS, defaultColormap } from './colormaps.mjs';
import {
...
import { MetricsPanel } from './metrics.mjs';

const DATA_ROOT = '../data/';

const RUNS = [
  { id: 'run-default', label: 'база · 10 кВ · 10 кГц · фото ВКЛ' },
  { id: 'run-nophoto', label: 'без фотопроцессов · 10 кВ · 10 кГц' },
  { id: 'run-low', label: 'низкое U · 6 кВ (диффузный)' },
  { id: 'run-high', label: 'высокое U · 14 кВ' },
  { id: 'run-fast', label: 'высокая f · 10 кВ · 30 кГц' },
  { id: 'run-synth', label: 'синтетика (отладка формата)' },
];
...
// графики (plots.mjs / metrics.mjs) — создаются заново на каждый прогон
const P = {
...
function timeUnit(span) {
  if (span >= 1e-3) return { k: 1e3, u: 'мс' };
  if (span >= 1e-6) return { k: 1e6, u: 'мкс' };
  if (span >= 1e-9) return { k: 1e9, u: 'нс' };
  return { k: 1e12, u: 'пс' };
}
function fmtTime(t) {
  if (t >= 1e-3) return `${(t * 1e3).toFixed(4)} мс`;
  if (t >= 1e-7) return `${(t * 1e6).toFixed(4)} мкс`;
  return `${(t * 1e9).toFixed(2)} нс`;
}
...
async function getRun(id, detail) {
  ...
  const pb = await Playback.load(dir, {
    ...
    onProgress: (got, total) => {
      ...
      setStatus(total ? `${id}: ${mb} МБ / ${(total / 1048576).toFixed(1)} МБ` : `${id}: ${mb} МБ`);
    },
  });
  if (!pb.frameCount) throw new Error(`прогон ${id}: в manifest.json ноль кадров`);
...
async function loadAll() {
  ...
  try {
    ...
    sceneA.title = state.compare ? 'ФОТО ВКЛ' : (RUNS.find((r) => r.id === state.runId)?.id || '');
    sceneA.subtitle = state.compare ? 'run-default' : '';
    if (pbB) { sceneB.setPlayback(pbB); sceneB.title = 'ФОТО ВЫКЛ'; sceneB.subtitle = 'run-nophoto'; }
    ...
    renderRunInfo();
    buildPlots(pbA);
    setStatus(`${pbA.manifest.runId} · ${pbA.frameCount} кадров · сетка ${pbA.grid.nrOut}×${pbA.grid.nzOut} · ${(pbA.manifest.stats.tLast * 1e6).toFixed(3)} мкс`);
  } catch (err) {
    pbA = null; pbB = null;
    destroyPlots();
    sceneA.setPlayback(null); sceneB.setPlayback(null);
    sceneA.error = 'данные не загружены';
    setStatus(String(err.message || err), true);
    showOverlay('Данные не загрузились', `${err.message || err}\n\nПроверьте, что каталог ${dirFor(state.compare ? 'run-default' : state.runId, state.detail)} существует и содержит manifest.json / frames.bin / series.bin,\nи что страница открыта через HTTP-сервер (file:// не работает из-за CORS).`);
  } finally {
    state.loading = false;
  }
}
...
function shortLabel(f) {
  switch (f.name) {
    case 'n_e': return 'n_e';
    case 'ionizRate': return 'свечение';
    case 'rho': return 'заряд ρ';
    case 'Emag': return '|E|';
    case 'EN': return 'E/N';
    case 'photoIonRate': return 'фотоионизация';
    case 'photoDetachRate': return 'фотоотлипание';
    case 'n_O3m': return 'O₃⁻';
    case 'n_O3': return 'O₃';
...
function drawTimeline() {
  ...
  drawTrack(ctx, box.A, a0, a1, { title: 'ВЕСЬ ПРОГОН', density: true, frames: false });
  // ── нижняя дорожка: лупа
  ...
  TL.zoom = [b0, b1];
  drawTrack(ctx, box.B, b0, b1, { title: 'ЛУПА НА ИМПУЛЬС (масштаб окна)', density: false, frames: true, local: true });
...
function drawTrack(ctx, b, t0, t1, opt) {
  ...
  if (s && s.t && s.t.length > 1) {
    ...
    ctx.fillText(`U ±${fmtSci(uMax)} В`, b.x1 - 4, y0 + 3);
    ctx.fillStyle = 'rgba(240,165,60,0.9)';
    ctx.fillText(`I ±${fmtSci(iMax)} А (asinh)`, b.x1 - 4, y0 + 14);
    if (opt.local) {
      ctx.fillStyle = '#59616e';
      ctx.fillText('нормировка по окну', b.x1 - 4, y0 + 25);
    }
  }
  ...
  if (opt.density) {
    ...
    ctx.fillText('кадры', b.x0 - 4, b.dens.y + b.dens.h / 2);
  }
...
const READOUT_ROWS = [
  ['t', (s, i) => fmtTime(s.t[i])],
  ['U(t), В', (s, i) => fmtSci(s.Uapp[i])],
  ['U на зазоре, В', (s, i) => fmtSci(s.Ugap[i])],
  ['I пров., А', (s, i) => fmtSci(s.Icond[i])],
  ['I смещ., А', (s, i) => fmtSci(s.Idisp[i])],
  ['I полн., А', (s, i) => fmtSci(s.Itot[i])],
  ['max E/N, Тд', (s, i) => fmtSci(s.maxEN[i])],
  ['σ max, Кл/м²', (s, i) => fmtSci(s.sigmaMax[i])],
  ['O₃, ppm', (s, i) => fmtSci(s.o3ppm[i])],
  ['фотоэмиссия L', (s, i) => fmtSci(s.photoEmitTotalL ? s.photoEmitTotalL[i] : NaN)],
  ['фотоэмиссия R', (s, i) => fmtSci(s.photoEmitTotalR ? s.photoEmitTotalR[i] : NaN)],
];
...
function renderRunInfo() {
  ...
  html += `U₀ = <b>${p.U0kV} кВ</b>, f = <b>${p.freqKHz} кГц</b><br>`;
  html += `фотопроцессы: <b>${p.photoModule ? 'ВКЛ' : 'ВЫКЛ'}</b><br>`;
  html += `сетка ${p.nr}×${p.nz} (запись ${m.grid.nrOut}×${m.grid.nzOut})<br>`;
  html += `кадров ${m.frames.length}, шагов ${m.stats.steps}<br>`;
  html += `t ∈ [0, ${(m.stats.tLast * 1e6).toFixed(4)} мкс]`;
  if (crash) {
    html += `<div class="crash">⚠ прогон оборван на пробое (t = ${(crash.t * 1e6).toFixed(4)} мкс):<br>${String(crash.message).slice(0, 420)}</div>`;
  }
  el.runInfo.innerHTML = html;
  el.limitNote.textContent = (m.limits && m.limits.note)
    || 'Осесимметричная модель описывает только центральный канал.';
}
...
function buildPlots(pb) {
  ...
  el.plotsSrc.textContent = state.compare ? '← run-default (левая сцена)' : pb.manifest.runId;
  el.photoStat.textContent = 'интегрирование по объёму…';
  P.pp.setData(null);
  PhotoSeriesBuilder.build(pb, {}).then((d) => {
    ...
    el.photoStat.textContent = d.missing && d.missing.length
      ? `нет каналов: ${d.missing.join(', ')}` : 'темп по объёму газа, эв./с';
  }).catch((e) => { if (my === P.token) el.photoStat.textContent = 'ошибка: ' + e.message; });
}
...
function updatePlots(t, i) {
  ...
  P.mp.update({
    ...
    specEnergy: { v: P.wcum ? P.wcum[k] / P.Vgas / 1000 : null, note: 'накопл. / V_gas' },
    ozone: s.o3ppm[k],
    maxEN: s.maxEN[k],
    Ipeak: P.ipk,
    breakdowns: P.breakdowns / Math.max(1, s.t[s.t.length - 1] / P.T),
    C_diel: fit && fit.ok && !fit.warn ? fit.C_diel : null,
    C_cell: fit && fit.ok ? { v: fit.C_cell, note: 'медиана dQ/dU' } : null,
    energy: fit && fit.ok ? { v: fit.W, note: fit.warn ? '⚠ петля не замкнута' : 'за период' } : null,
    _status: {
...
function composite(cv) {
  ...
  g.fillText(`${runTag()} · ${f ? f.label : state.field} · t = ${fmtTime(state.t)}`
    + ` · ${state.logScale ? 'log' : 'lin'} · шкала: ${state.scaleMode === 'global' ? 'весь прогон' : 'кадр'}`,
  8, cap / 2);
  return cv;
}
...
function exportPNG() {
  ...
  setStatus(`PNG сохранён · ${state.field} · ${fmtTime(state.t)}`);
}
...
function exportCSV() {
  ...
  setStatus(`CSV: ${n} строк × ${names.length} колонок`);
}
...
function toggleWebM() {
  ...
  if (!pbA) return;
  if (typeof MediaRecorder === 'undefined') { setStatus('MediaRecorder не поддерживается браузером', true); return; }
  ...
  REC.mr.onstop = () => {
    ...
    setStatus(`WebM записан (${(REC.chunks.reduce((a, b) => a + b.size, 0) / 1048576).toFixed(1)} МБ)`);
  };
  REC.mr.start(200);
  ...
  el.expWebm.textContent = '⏹ стоп';
  setStatus('запись WebM…');
}
function stopWebM() {
  state.recording = false;
  el.expWebm.classList.remove('rec');
  el.expWebm.textContent = '⏺ WebM';
...
function frameLoop(ts) {
  ...
  if (pbA) {
    ...
    const i = curIndex();
    el.clock.textContent = `${fmtTime(state.t)}  ·  кадр ${i + 1}/${pbA.frameCount}`;
    el.loopInfo.textContent = state.loop
      ? `цикл ${fmtTime(state.loop[0])} … ${fmtTime(state.loop[1])}` : '';
  } else {
    sceneA.render();
  }
...
function initControls() {
  for (const r of RUNS) {
    ...
    o.value = r.id; o.textContent = `${r.id} — ${r.label}`;
    el.runSel.appendChild(o);
  }
  el.runSel.value = state.runId;
  for (const c of COLORMAP_NAMES) {
    const o = document.createElement('option');
    o.value = c; o.textContent = COLORMAP_LABELS[c] || c;
