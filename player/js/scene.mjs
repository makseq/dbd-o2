// scene.mjs — главная сцена: осесимметричный срез (r,z), зеркально отражённый
...
import { getLUT, isDiverging } from './colormaps.mjs';
...
export class Scene {
  ...
  _normalize() {
    ...
    if (isSigned) {
      ...
      range = { kind: 'sym', vmax: amax, v0, unit: spec.unit };
    } else {
      ...
      if (this.opts.logScale) {
        ...
        if (!(vmax > 0)) { out.fill(0); range = { kind: 'log', vmin: 0, vmax: 0, unit: spec.unit }; }
        else {
          ...
          range = { kind: 'log', vmin, vmax, unit: spec.unit };
        }
      } else {
        if (!(vmax > 0)) { out.fill(0); range = { kind: 'lin', vmin: 0, vmax: 0, unit: spec.unit }; }
        else {
          const inv = 1 / vmax;
          for (let k = 0; k < n; k++) out[k] = Math.min(1, Math.max(0, dec[k] * inv));
          range = { kind: 'lin', vmin: 0, vmax, unit: spec.unit };
        }
      }
    }
    this.range = range;
  ...
  _drawProfile(ctx, L) {
    ...
    const rightTxt = `max = ${fmtSci(vMax)} при z = ${this.zCenters[jMax].toFixed(3)} мм`;
    const leftTxt = 'профиль на оси r = 0';
    ...
    if (Number.isFinite(rHalf)) {
      ...
      ctx.fillText(`r½ = ${(rHalf * 1000).toFixed(1)} мкм по r`, xb - 5, y0 + 17);
    }
    // отметка максимума
  ...
  render() {
    ...
    if (!this.pb) {
      ...
      ctx.fillText(this.error || 'нет данных', w / 2, h / 2);
      return;
    }
  ...
  _drawSolids(ctx, L) {
    ...
    for (const d of this.dielectrics) {
      ...
      if (xb - xa > 74) {
        ...
        ctx.fillText(`${(d.z1 - d.z0).toFixed(1)} мм`, cx, yT + L.H / 2 + 20);
      }
      ctx.restore();
    }
    ...
    ctx.textAlign = 'left';
    ctx.fillText('МЕТАЛЛ · U(t)', this.X(L, 0) - bar, yT - 10);
    ctx.textAlign = 'right';
    ctx.fillText('МЕТАЛЛ · ⏚ 0 В', this.X(L, this.Lz) + bar, yT - 10);
  }
  ...
  _drawGas(ctx, L) {
    ...
    ctx.fillText('газ O₂ · 1 атм · зазор 1.0 мм', (xa + xb) / 2, L.y0 - 10);
    ctx.save();
    ctx.fillStyle = 'rgba(232,234,237,0.5)';
    ctx.font = '9px Inter, system-ui, sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('ось r = 0', xa + 5, this.Y(L, 0) - 4);
    ctx.restore();
  }
  ...
  _drawFrameAndAxes(ctx, L) {
    ...
    ctx.fillText('z, мм', L.x0 + L.W / 2, L.y0 + L.H + 30);
    // ось r
  ...
  _drawColorbar(ctx, L, w, h) {
    ...
    ctx.font = '9px Inter, system-ui, sans-serif';
    const modeTxt = (range.kind === 'log' ? 'лог' : range.kind === 'sym' ? 'asinh ±' : 'лин')
      + (this._forceMax ? ' · шкала синхр.' : '');
  ...
  _drawHUD(ctx, L, w, h) {
    ...
    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    const head = this.title ? `${this.title} · ${spec.label}` : spec.label;
    ctx.fillText(head, 8, 16);
    ctx.font = '10px "JetBrains Mono", ui-monospace, monospace';
    ctx.fillStyle = CSS.dim;
    ctx.textAlign = 'right';
    const t = fr ? fr.t : 0;
    ctx.fillText(`t = ${(t * 1e6).toFixed(4)} мкс · кадр ${this.frame + 1}/${this.pb.frameCount}`, w - 8, 16);
    if (this.opts.afterglow) {
      ctx.textAlign = 'left';
      ctx.fillStyle = 'rgba(250,178,25,0.9)';
      ctx.fillText('послесвечение', 8, 30);
    }
    if (this.subtitle) {
  ...
  _drawProbe(ctx, L) {
    ...
    let txt = `r = ${p.r.toFixed(3)}  z = ${p.z.toFixed(3)} мм`;
    let val = null;
    if (p.z >= this.zGas0 && p.z <= this.zGas1 && this.decoded) {
      ...
      txt += `   σ = ${fmtSci(val)} Кл/м²`;
      val = null;
    }
    if (val !== null) txt += `   ${fmtSci(val, 3)} ${this.fieldSpec().unit}`;
    ctx.font = '10px "JetBrains Mono", ui-monospace, monospace';
