// loader.mjs — загрузка прогона ДБР (manifest.json + frames.bin + series.bin) в браузере.
//
// Это КАНОНИЧЕСКИЙ ДЕКОДЕР формата: recorder.mjs кодирует, этот файл декодирует,
// тест recorder.test.mjs гоняет round-trip именно через него.
//
// Обратные формулы квантования (парные к recorder.mjs):
//   log   : v = (q === 0) ? 0 : 10^(log10(min) + q/scale)      // min = max*10^-decades
//   lin   : v = q / scale                                       // scale = qmax/max
//   asinh : v = v0 * sinh(q / scale)                            // q = 0 -> ТОЧНЫЙ ноль
//
// Модуль без зависимостей; в node работает при передаче своего fetch:
...
export function decodeField(q, codec, spec, out) {
  ...
  if (map === 'log') {
    ...
    throw new Error(`loader: неизвестный map '${map}'`);
  }
  return dst;
}
...
function readHeader(buf, magic) {
  ...
  if (s !== magic) throw new Error(`loader: неверная сигнатура '${s.replace(/\0/g, '')}', ожидалась '${magic.replace(/\0/g, '')}'`);
  return {
...
export class Playback {
  ...
  static async load(url, opts = {}) {
    ...
    if (!fetchFn) throw new Error('loader: нет fetch — передайте opts.fetch');
    ...
    if (!res.ok) throw new Error(`loader: manifest.json -> HTTP ${res.status} (прогон не завершён?)`);
    const manifest = await res.json();
    if (manifest.formatVersion !== SUPPORTED_FORMAT_VERSION) {
      throw new Error(`loader: версия формата ${manifest.formatVersion}, поддерживается ${SUPPORTED_FORMAT_VERSION}`);
    }
    ...
    if (pb.mode === 'eager') {
      ...
      if (pb.framesBuf.byteLength !== expect) {
        throw new Error(`loader: frames.bin ${pb.framesBuf.byteLength} Б, по манифесту ${expect} Б`);
      }
      if (pb.header.count !== manifest.frames.length) {
        throw new Error(`loader: заголовок frames.bin: ${pb.header.count} кадров, манифест: ${manifest.frames.length}`);
      }
    }
  ...
  async loadSeries() {
    ...
    if (h.count !== s.count || h.stride !== s.recordStride) {
      throw new Error(`loader: series.bin рассинхронизирован с манифестом (${h.count}/${h.stride} против ${s.count}/${s.recordStride})`);
    }
    const dv = new DataView(buf);
  ...
  _frameBytes(i) {
    ...
    if (this.mode === 'eager') {
      ...
      if (off + fl.frameStride > this.framesBuf.byteLength) throw new Error(`loader: кадр ${i} выходит за пределы frames.bin`);
      return { buf: this.framesBuf, off };
    }
    const blk = this.blocks && this.blocks.get(i);
    if (!blk) throw new Error(`loader: кадр ${i} ещё не загружен — вызовите await getFrameAsync(${i}, ...) или prefetch()`);
  ...
  async fetchFrameBlock(i) {
    ...
    if (!res.ok) throw new Error(`loader: Range-запрос кадра ${i} -> HTTP ${res.status}`);
    const ab = await res.arrayBuffer();
    if (ab.byteLength !== fl.frameStride) {
      throw new Error(`loader: сервер вернул ${ab.byteLength} Б вместо ${fl.frameStride} (Range не поддержан?)`);
    }
  ...
  getFrame(i, field) {
    if (i < 0 || i >= this.frameCount) throw new Error(`loader: кадра ${i} нет (всего ${this.frameCount})`);
    const key = `${i}|${field}`;
    const hit = this.cache.get(key);
    if (hit) return hit;
    const spec = this.fieldByName.get(field);
    if (!spec) throw new Error(`loader: поля '${field}' нет в прогоне`);
